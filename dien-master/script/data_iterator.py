...
class DataIterator:
    ...
    def __init__(self, source,  # file path
        ...
        for line in f_review:
            arr = line.strip().split("\t")  # userID + "\t" + itemID + "\t" + str(rating) + "\t" + str(time)

            tmp_idx = 0
            if arr[1] in self.source_dicts[1]:
                tmp_idx = self.source_dicts[1][arr[1]]
            self.mid_list_for_random.append(tmp_idx)  # 评论中的item的index
    ...
    def next(self):
        ...
        if len(self.source_buffer) == 0:
            for k_ in range(self.k):
                # label, uid, itemId, cat, hist_itemId, hist_cat
            ...
            if self.sort_by_length:
                ...
                self.source_buffer.reverse()  # 为什么要倒转呢？
            ...
            while True:
                ...
                for pos_mid in mid_list:
                    ...
                    while True:
                        ...
                        if noclk_index >= 5:
                            break
                    noclk_mid_list.append(noclk_tmp_mid)  # 对于每个历史记录中的item，取5个负样本？
                    noclk_cat_list.append(noclk_tmp_cat)
                source.append([uid, mid, cat, mid_list, cat_list, noclk_mid_list, noclk_cat_list])
                target.append([float(ss[0]), 1-float(ss[0])])  # [1, 0]正样本，[0, 1]负样本
