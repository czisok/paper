...
iddd = 0
for line in f_train:
    arr = line.strip("\n").split("\t")  # label, uid, itemId, cat, hist_itemId, hist_cat
    clk = arr[0]
    uid = arr[1]
    mid = arr[2]
    cat = arr[3]
...
pkl.dump(uid_voc, open("uid_voc.pkl", "w"))
pkl.dump(mid_voc, open("mid_voc.pkl", "w"))
pkl.dump(cat_voc, open("cat_voc.pkl", "w"))
