...
#     for key in user_map:    # key: uid
#         sorted_user_bh = sorted(user_map[key], key=lambda x:x[1])  # 将一个用户的记录通过时间排序(uid,itemId,rating,time)
#         for line, t in sorted_user_bh:
#             items = line.split("\t")
#             asin = items[1]  # item_id
#             j = 0
#             while True:
...
#                     continue 
#                 items[1] = asin_neg
#                 print>>fo, "0" + "\t" + "\t".join(items) + "\t" + meta_map[asin_neg]  # (0,uid, itemId, rating,time,cat)
#                 j += 1
#                 if j == 1:             #negative sampling frequency
#                     break
#             if asin in meta_map:
#                 print>>fo, "1" + "\t" + line + "\t" + meta_map[asin]  # (1, uid, itemId, rating,time,cat)
#             else:
...
#                 print>> fo, "20180118" + "\t" + line
#             else:
#                 print>>fo, "20190119" + "\t" + line  # 每个用户的最后两个样本放到‘20190119’里面
#         i += 1
...
def process_meta(file):
    ...
    for _ in tnrange(count):
        obj = eval(fi.readline())
        cat = obj["categories"][0][-1]
...
def manual_join():
    ...
    count = 0
    while f_rev.readline():
        count += 1
    f_rev.seek(0)
    
    for _ in tnrange(count):
        line = f_rev.readline().strip()
        items = line.split("\t")
        #loctime = time.localtime(float(items[-1]))
        #items[-1] = time.strftime('%Y-%m-%d', loctime)
        if items[0] not in user_map:
    ...
    count = 0
    while f_meta.readline():
        count += 1
    f_meta.seek(0)
    
    for _ in tnrange(count):
        arr = f_meta.readline().strip().split("\t")
        if arr[0] not in meta_map:
            meta_map[arr[0]] = arr[1]
            arr = line.strip().split("\t")
    fo = open("jointed-new", "w")
        ...
        for line, t in sorted_user_bh:
            ...
            while True:
                ...
                if asin_neg == asin:
                    continue 
                items[1] = asin_neg
                fo.write("0" + "\t" + "\t".join(items) + "\t" + meta_map[asin_neg]+'\n')  # (0,uid, itemId, rating,time,cat)
                j += 1
                if j == 1:             #negative sampling frequency
                    break
            if asin in meta_map:
                print>>fo.write("1" + "\t" + line + "\t" + meta_map[asin]+'\n')  # (1, uid, itemId, rating,time,cat)
            else:
                print>>fo.write("1" + "\t" + line + "\t" + "default_cat\n")
...
def split_test():
    ...
    for line in fi:
        ...
        if user == last_user:
            if i < user_count[user] - 2:  # 1 + negative samples
                print>> fo.write("20180118" + "\t" + line+'\n')
            else:
                print>>fo.write("20190119" + "\t" + line+'\n')
        else:
            last_user = user
            i = 0
            if i < user_count[user] - 2:
