...
def prepare_data(input, target, maxlen = None, return_neg = False):
    ...
    lengths_x = [len(s[4]) for s in input]      # 每个样本的历史长度
    seqs_mid = [inp[3] for inp in input]        # 每个样本的预测值
    seqs_cat = [inp[4] for inp in input]        # 每个样本预测值的类别
    noclk_seqs_mid = [inp[5] for inp in input]  # 每个样本历史中的负样本
